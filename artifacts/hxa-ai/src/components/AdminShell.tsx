...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-[100dvh] bg-[#0a1323] text-[#dbeaff]">
    ...
    <div className="md:pl-64"><header className="flex h-20 items-center justify-between border-b border-[#3f5b7e]/30 px-5 sm:px-8"><button onClick={() => setOpen(true)} data-testid="button-open-sidebar" className="rounded-lg border border-[#486486]/40 p-2 text-[#9cb5d2] md:hidden"><Menu className="h-5 w-5" /></button><div className="hidden text-xs uppercase tracking-[.16em] text-[#6885a8] md:block">HXA / Control room</div><div className="ml-auto flex items-center gap-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-[#4286f5]/20 text-xs font-semibold text-[#98caff]">AM</div><span className="hidden text-sm text-[#a8bdd6] sm:block">Admin workspace</span></div></header><main className="mx-auto max-w-[1400px] p-5 sm:p-8">{children}</main></div>
  </div>;
}