...
export default function ProcessDetailPage({ stepId }: { stepId: string }) {
  ...
  return <div className="hxa-noise min-h-[100dvh] bg-[#08101f] text-[#e9f3ff]">
    <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-8"><BrandMark /><Link href="/#capabilities" className="inline-flex items-center gap-2 text-sm text-[#a7bdd6] hover:text-[#e9f3ff]"><ArrowLeft className="h-4 w-4" />Back to process</Link></header>
    <main className="mx-auto max-w-7xl px-6 pb-24 pt-16 lg:px-8 lg:pb-36 lg:pt-24">
      ...
      <section className="mt-20 max-w-4xl rounded-2xl border border-[#456485]/35 bg-[#0d192b]/70 p-7 sm:p-10"><div className="eyebrow">What happens here</div><ul className="mt-8 space-y-5">{detail.points.map((point) => <li key={point} className="flex items-center gap-3 text-lg text-[#c8d9eb]"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#4286f5]/15 text-[#7ebcff]"><Check className="h-3.5 w-3.5" /></span>{point}</li>)}</ul><Link href="/#contact" className="mt-10 inline-flex items-center gap-2 rounded-full bg-[#4286f5] px-5 py-3 text-sm font-semibold text-[#071226] hover:bg-[#83bdff]">Talk to AddMatix <ArrowUpRight className="h-4 w-4" /></Link></section>
    </main>
  </div>;
}