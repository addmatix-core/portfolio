...
export default function ServiceDetailPage({ serviceId }: { serviceId: string }) {
  ...
  return <div className="hxa-noise min-h-[100dvh] bg-[#08101f] text-[#e9f3ff]">
    <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-8">
      <BrandMark />
      <Link href="/#capabilities" className="inline-flex items-center gap-2 text-sm text-[#a7bdd6] transition-colors hover:text-[#e9f3ff]"><ArrowLeft className="h-4 w-4" />Back to services</Link>
    </header>
    <main className="mx-auto max-w-7xl px-6 pb-24 pt-16 lg:px-8 lg:pb-36 lg:pt-24">
      ...
      <div className="mt-16 rounded-2xl border border-[#456485]/35 bg-[#0d192b]/70 p-8 sm:p-10"><div className="max-w-2xl"><div className="eyebrow">Make the next move</div><h2 className="mt-4 font-display text-3xl tracking-[-.05em] text-[#e7f3ff]">Have a signal worth following?</h2><p className="mt-3 text-sm leading-6 text-[#91a5bd]">Tell us where you are headed. We will bring a point of view, not a pitch.</p><Link href="/#contact" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#4286f5] px-5 py-3 text-sm font-semibold text-[#071226] transition-colors hover:bg-[#83bdff]">Start a conversation <ArrowUpRight className="h-4 w-4" /></Link></div></div>
    </main>
  </div>;
}